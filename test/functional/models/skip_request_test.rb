...
class SkipRequestTest < Test::Unit::TestCase
  ...
  context "consider" do
    should "create a new skip request when there are none" do
    ...
    should "not create a new skip request when there is one from same day and ip address" do
      entry = PlaylistEntry.create! :file_location => 'some_location', :status => PlaylistEntry::PLAYING
    ...
    should "create a new skip request when there is one from the same day but different ip address" do
      entry = PlaylistEntry.create! :file_location => 'some_location', :status => PlaylistEntry::PLAYING
      Time.is(Time.parse("Sun Mar 14 12:00:00")) do
    ...
    should "create a new skip request when there is one from the same day and same ip address but different songs" do
      entry = PlaylistEntry.create! :file_location => 'some_location', :status => PlaylistEntry::PLAYING
      Time.is(Time.parse("Sun Mar 14 12:00:00")) do
    ...
    should "purge old skip requests that are not from today and create a new skip request" do
      entry = PlaylistEntry.create! :file_location => 'some_location', :status => PlaylistEntry::PLAYING
      Time.is(Time.parse("Sun Mar 14 12:00:00")) do
        SkipRequest.create! :requested_by => "127.0.0.1", :file_location => 'some_other_location'
      end
